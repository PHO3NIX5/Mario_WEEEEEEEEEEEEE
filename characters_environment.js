...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press The Start Button to Start", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if((Game_Status=="Start" && X!=0)&& gameConfig.status==="start") {
    world_start.play();
...
function jumping(character){
	if( (Y<200&&character.live) || (touchIsDown&&character.live) ){
